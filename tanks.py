...
'''
create my PriorityQueue
'''
import heapq

class MyPriorityQueue(object):
    def __init__(self):
        self.heap = []          #init a empty list as my heap

    def add(self, d, pri):
        heapq.heappush(self.heap, (pri, d))     #push (node, node.f)
...
class Bonus():
    ...
    def __init__(self, level):
        ...
        '''
        Originally the bonus will appear any random position, now we fixed the position bonus appeared on (200, 350, 32, 32).
        And we simplified the game with only bonus BONUS_SHOVEL appeared.
        '''
        self.rect = pygame.Rect(200, 350, 32, 32)

        self.bonus = self.BONUS_SHOVEL
        '''
...
class Level():
    ...
    def __init__(self, level_nr = None):
        ...
        global sprites
        '''
        Here we set the enemy tank appeared one by one. We want to display the path founded for
        the enemy tank in path_display window.
        '''
        ...
        for tile in self.mapr:
            if tile[1].topleft == pos:
                if tile[0] == self.TILE_BRICK:
                    ...
                    astar_map[int(round(pos[1]/16))][int(round(pos[0]/16))]=1       #update astar_map
                    self.updateObstacleRects()
                    ...
                    if power == 2:
                        self.mapr.remove(tile)
                        astar_map[round(pos[1]/16)][round(pos[0]/16)]=1             #update astar_map
                        self.updateObstacleRects()
                    return True
                else:
                    return False
    ...
    def draw(self, tiles = None): # draw level map , Level class
        ...
        for tile in self.mapr:
            if tile[0] in tiles:
                ...
                #we draw display window in the right side for displaying path founded
                if ai.TEST_AI is True:
                    topleft_forAI = [tile[1].topleft[0] + 480, tile[1].topleft[1]]
                    ai.generate_map_forai(screen, topleft_forAI)
...
class Enemy(Tank):
    ...
    def __init__(self, level, type, position = None, direction = None, filename = None):
        ...
        self.directions = [self.DIR_UP, self.DIR_RIGHT, self.DIR_DOWN, self.DIR_LEFT]
    ...
    def spawnBonus(self):
        ...
        gtimer.add(1, lambda :bonus.toggleVisibility())
    ...
    def move(self):
        ...
        global players, enemies, bonuses, astar_map

        if self.state != self.STATE_ALIVE or self.paused or self.paralised:
            return

        #if(self.path == []):
        if self.rect.top % 16 == 3 and self.rect.left % 16 == 3 or self.path == []:
            self.path = self.generatePath(direction=None, fix_direction=False)
        if self.path == None:
            return
        if len(self.path) < 2:
            return
        ...
        if next_position[0] > new_position[0] and self.direction is not self.DIR_RIGHT:
            self.rotate(self.DIR_RIGHT)
        elif next_position[0] < new_position[0] and self.direction is not self.DIR_LEFT:
            self.rotate(self.DIR_LEFT)
        if next_position[1] > new_position[1] and self.direction is not self.DIR_DOWN:
            self.rotate(self.DIR_DOWN)
        if next_position[1] < new_position[1] and self.direction is not self.DIR_UP:
            self.rotate(self.DIR_UP)
        ...
        '''
        Collision detections, remove for now. maybe needed
    ...
    '''============================================================================
        The following methods are created to generate path by A star algorithm.
    ============================================================================'''
    ...
    def generatePath(self, direction=None, fix_direction=False):
        ...
        if shovelTrigger == False:
            ai.update_astarMap(astar_map=astar_map, player_position=players[0].rect.topleft, recover=False)

        startPixPosition = [self.rect.left, self.rect.top]
        startPosition = [int(round(self.rect.left / 16)), int(round(self.rect.top / 16))]
        pixDiff = [startPixPosition[0] - startPosition[0] * 16, startPixPosition[1] - startPosition[1] * 16]

        if shovelTrigger == True:
            goalPosition = [int(round(players[0].rect.topleft[0] / 16)),int(round(players[0].rect.topleft[1] / 16)) ]
        else:
            if (self.rect.left < 208):
                goalPosition = [12, 24]
            else:
                goalPosition = [13, 24]
        ...
        while (pq.isEmpty() == False):
            ...
            if currentPosition[0] == goalPosition[0] and currentPosition[1] == goalPosition[1]:
                ...
                if shovelTrigger == False:
                    ai.update_astarMap(astar_map=astar_map, player_position=players[0].rect.topleft, recover=True)
                return self.fromNodesToPositions(pathNodes, pixDiff)

            for direction in self.directions:

                if direction == self.DIR_UP and currentPosition[1] > 1:
                    newPosition = [currentPosition[0], currentPosition[1] - 1]
                elif direction == self.DIR_RIGHT and currentPosition[0] < 24:
                    newPosition = [currentPosition[0] + 1, currentPosition[1]]
                elif direction == self.DIR_DOWN and currentPosition[1] < 24:
                    newPosition = [currentPosition[0], currentPosition[1] + 1]
                elif direction == self.DIR_LEFT and currentPosition[0] > 1:
                    newPosition = [currentPosition[0] - 1, currentPosition[1]]
                else:
                    continue

                if self.visited_map[newPosition[1]][newPosition[0]] == 1:
                    continue

                if astar_map[newPosition[1]][newPosition[0]] == 0:
                    continue

                childNode = Node(currentNode.getG(), currentNode, newPosition)

                self.visited_map[newPosition[1]][newPosition[0]] = 1

                g = self.cost(childNode)

                childNode.setG(g)

                childNode.setF(childNode.getG() + self.manhattanDistance(childNode, goalPosition))
                pq.add(childNode, childNode.getF())
    ...
    def fromNodesToPositions(self, pathNodes, pixDiff):
        ...
        x_pix_cur = self.rect.left
        y_pix_cur = self.rect.top
        offset = 0
        ...
        for i in range(len(pathNodes) - 1):
            ...
            if diffX == 0: # moving up or down
                if diffY > 0: # moving down
                    for num in range(0, nextPosition[1] * 16 - y_pix_cur, self.speed):
                        positions.append([x_pix_cur + offset, y_pix_cur + num + offset])
                else:# moving up
                    for num in range(0, y_pix_cur - nextPosition[1] * 16, self.speed):
                        positions.append([x_pix_cur + offset, y_pix_cur - num + offset])
            if diffY == 0: # moving left or right
                if diffX > 0: # moving right
                    for num in range(0, nextPosition[0] * 16 - x_pix_cur, self.speed):
                        positions.append([num + x_pix_cur + offset, y_pix_cur + offset])
                else: # moving left
                    for num in range(0 , x_pix_cur - nextPosition[0] * 16, self.speed):
                        positions.append([x_pix_cur - num + offset, y_pix_cur + offset])

            x_pix_cur = nextPosition[0] * 16
            y_pix_cur = nextPosition[1] * 16
    ...
    def cost(self, node):
        ...
        astar_cost = astar_map[position[1]][position[0]] + astar_map[position[1] + 1][position[0]] \
                     + astar_map[position[1]][position[0] + 1] + astar_map[position[1] + 1][position[0] + 1]

        return parentCost + astar_cost

    def expandNode(self,nodeToBeExpand, goalPosition):
    ...
    '''==========================================================================================
    Above several methods are used to generate path by A star Algorithm.
    ============================================================================================='''
...
class Player(Tank):
    ...
    def player_updatePlayerMap(self):
        global player_map
        player_map = [[0 for x in range(26)] for x in range(26)]
        x = self.mapPositionOfPlayer[0]
        y = self.mapPositionOfPlayer[1]
...
class Game():
    ...
    def triggerBonus(self, bonus, player):
        ...
        global enemies, labels, play_sounds, sounds, shovelTrigger
    ...
    def nextLevel(self):
        ...
        levels_enemies = (
            (0,10,0,0), (14,4,0,2), (14,4,0,2), (2,5,10,3), (8,5,5,2),
...
if __name__ == "__main__":
    ...
    sounds = {}
    '''
    define matrix for astar_map.
    help to display path founded in the right side window.
    '''
    ...
    '''
    we add an test_start() method in class game() to start the game with A star algorithm we added.
    showMenu() method will start the game without A star algorithm (most of its original settings).
    '''
    if ai.TEST_AI is True:
        game.test_start()
    else:
